...
export default function Produtos({ data }: any) {
    ...
    const fresasInsertadas = tools.filter((tool: any) => tool.categories.includes(50))
                    ...
                    <div className="row">
                        ...
                        <div className="col-12">
                            <div className={styles.pageIndication}>
                                ...
                                <a href="#">Escariadores</a>
                            </div>
                        </div>

                        <div className="col-12">
                            <div className={styles.products}>
                                {fresasInsertadas.map(tool =>(
                                    <div className={styles.productsBox} key={tool.id}>
                                        <div className={styles.productsImg}>
                                            <img 
                                                width="100%" 
                                                height="90" 
                                                src={tool._embedded?.['wp:featuredmedia']?.[0]?.source_url || '/ferramenta.jpg'} 
                                                alt=""
                                            />
                                        </div>
...
export const getStaticProps: GetStaticProps = async () => {
    const { data } = await api.get('tools?per_page=100&_embed&categories=50&orderby=title&order=asc',  {
        params: {
            _limit: 12,
            _order: 'desc'
        }
